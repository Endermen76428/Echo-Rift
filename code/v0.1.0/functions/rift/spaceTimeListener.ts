...
const cacheRifts: { [key: string]: IRiftInfo } = {} // Entity Rift Id > Hold Info
let amountOfListeners = 0
let invalidRifts = 0
const maxTick = 9 // É o 10° Tick
...
function startInverval(executeTime = 0): void {
  ...
  for(let i = 0; i < length; i++){
    ...
    if(executeTime != maxTick) continue
    ...
    for(let playerI = 0, playerLen = players.length; playerI < playerLen; playerI++){
      ...
      if(!player || !player.isValid || !(player instanceof Player)) continue

      if(player.hasTag(riftTag)) continue
      ...
      if(current != 0){
        ...
        } else apiWarn.notify(player, "entity.warn.echo_rift:spacetime_rift.unstable.complete", {type: "action_bar", sound: "warn.ender_addon_pack:orb"})
      }
    }
    ...
    if(current != 0){
      ...
      for(let itemI = 0, itemLen = items.length; itemI < itemLen; itemI++){
        ...
        if(!itemEntity || deny.has(itemEntity.id)) continue
      ...
      if(current != info.current){
        ...
        if(percentage != 100){
          // console.warn("Tocar o som de colocar glowstone na ancora de renascimento")
    ...
    if(rift.getProperty("echo_rift:closing") == false) rift.triggerEvent("echo_rift:start_close")
    ...
    for(let playerI = 0, playerLen = players.length; playerI < playerLen; playerI++){
      ...
      if(player.hasTag(joinTag) && player.hasTag(riftTag)) continue
      if(!(player instanceof Player)) continue

      if(Math.floor(player.location[displacement]) != riftAxis) continue
      const distance = riftOppositeAxis - player.location[oppositeDisplacement]
      if(distance * distance > width) continue
...
export function addRiftListener(entity: Entity): void {
  ...
  let current = 0
  const items: TItemRequest[] = Array.from({length: 4})
  for(let i = 0; i < 4; i++){
    const item = entity.getDynamicProperty(`item${i}`)
    if(typeof item != "string") return // Tem que ser return porque sempre precisa ser 4 itens
  ...
  const emptyItem: TItemRequest = {item: "", amount: 0}
...
export function removeRiftListener(entityId: string, entity?: Entity): void {
...
function decrementItem(entity: Entity, item: ItemStack, amount: number): number {
