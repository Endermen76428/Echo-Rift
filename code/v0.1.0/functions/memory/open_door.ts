import { Block, BlockPermutation, BlockVolume, VectorXZ } from "@minecraft/server"
import { MaxDoorSize } from "../../lib/variables"
import { generateNextRoom } from "./rooms/default"
import { roomsCache } from "./roomCache"

export function openDoor(block: Block): void {
  ...
  for(let i = 1, len = maxW; i < len; i++){
    ...
    if(checkBlock == undefined || checkBlock.typeId != "echo_rift:door") break
  ...
  // console.warn("Vai ter a animação da entidade")
  ...
  if(exits && exits.length > 1){
    for(let i = 0, len = exits.length; i < len; i++){
      ...
      if(minCorner.x == exit.x && minCorner.y == exit.y && minCorner.z == exit.z) continue

      const offset = dirOffset[exit.dir]
      if(offset == undefined) continue

      // console.warn("Porta:", exit.x, exit.y, exit.z, exit.dir, JSON.stringify(offset))
