import { BlockVolume, Dimension, Entity, EntityComponentTypes, EntityDieAfterEvent, EntityHurtAfterEvent, world } from "@minecraft/server"
...
const entityDie: { [key: string]: (arg0: EntityDieAfterEvent) => void } = {}
const entityHurt: { [key: string]: (arg0: EntityHurtAfterEvent) => void } = {}

export function startEventType0KillAll(dimension: Dimension, reload = false): void {
  // console.warn("Event: §cKill all mobs.")
  ...
  if(spawnedEntities.length == 0) return

  score.setScore("enemies", spawnedEntities.length)
  if(reload == false) score.setScore("rg", life *1000) // Room Goal - A quantia máxima de vida de todas as entidades somadas que precisa ser retirada para concluir a fase

  const eventHurtId = world.afterEvents.entityHurt.subscribe(({hurtEntity, damage}) => {
    if(!hurtEntity.isValid || !hurtEntity.hasTag("echo_rift:need_kill")) return
  ...
  const eventDieId = world.afterEvents.entityDie.subscribe(({deadEntity, damageSource}) => {
    if(!deadEntity.isValid || !deadEntity.hasTag("echo_rift:need_kill")) return
    score.addScore("enemies", -1)
    if(score.getScore("enemies") == 0){
      roomInfo.exits && generateDoors(dimension, roomInfo.exits)

      score.setScore("rg", life *1000) // Estabiliza a barra no nivel maximo pois as vezes tem uma mini porcentagem que não é contada
...
const mobsList: { [key: string]: [IMobInfo[], IMobInfo[], IMobInfo[], IMobInfo[]] } = {
...
const minibossList: { [key: string]: [IMobInfo[], IMobInfo[], IMobInfo[], IMobInfo[]] } = {
  "echo_rift:forgotten_kingdom": [
    [ // Level 1
