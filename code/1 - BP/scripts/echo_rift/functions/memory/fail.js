...
export function memoryRaidFailed(player, healthComp, hungerComp) {
        ...
        if (backup != undefined) {
            ...
            if (playerInv && playerEquip && entityInv) {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    if (equip == undefined)
                        continue;
                    playerEquip.setEquipment(equip, entityInv.getItem(invSize - i - 1));
                }
            }
        }
        else {
            console.error("§cPlayer Inventory Backup Invalid!");
        }
    }, 5);
    system.runTimeout(() => {
