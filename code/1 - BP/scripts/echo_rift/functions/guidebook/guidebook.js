import { guidebookInfo } from "./guidebookPages";
...
export const guidebook = new class Guidebook {
    open(player, directory) {
        ...
        const form = new ActionFormData()
            .title(title);
        if (currentForm.body != false)
            form.body(translateId + "body." + titleId + (titlePath.length > 0 ? "." : "") + (currentForm.body ?? currentForm.title));
        if (currentForm.header)
            form.header(translateId + "header." + titleId + (titlePath.length > 0 ? "." : "") + currentForm.header);
        if (currentForm.labelB)
            form.label(translateId + "label." + titleId + (titlePath.length > 0 ? "." : "") + currentForm.labelB);
        buttons?.forEach((button, index) => {
            if (button.dividerB)
                form.divider();
            if (button.labelB)
                form.label(translateId + "label." + button.labelB);
            if (button.title) {
            ...
            if (button.labelA)
                form.label(translateId + "label." + button.labelA);
            if (button.dividerA)
                form.divider();
        });
    ...
    getDirectory(indexArray) {
        let current = { ...guidebookInfo };
        const path = [];
