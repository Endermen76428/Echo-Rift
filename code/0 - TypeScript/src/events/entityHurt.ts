import { Dimension, EntityComponentTypes, EntityDamageCause, EquipmentSlot, Player, system, world } from "@minecraft/server"
...
world.beforeEvents.entityHurt.subscribe(ev => {
  ...
  if(entity.typeId == "minecraft:player" && entity instanceof Player && entity.dimension.id.startsWith("echo_rift:")){
    ...
    if(equippableComp == undefined) return
    ...
    if(damageSource.cause != EntityDamageCause.void && (mainHand == totemId || offHand == totemId)) return
    ...
    if(healthComp == undefined || hungerComp == undefined || equippableComp == undefined) return
