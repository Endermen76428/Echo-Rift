import { Dimension, EntityComponentTypes, Player, ScoreboardObjective, system } from "@minecraft/server"
...
const inversePorcentage = 1000 / 5 // 1000 é 100% e 29 é a quantia de salas
const timerIntervalId: { [key: string]: IIntervalInfo } = {} // Dimension Id > Current Time in seconds
...
function runInterval(execution = 0){
    ...
    if(execution == 4){
      ...
      for(let i2 = 0, len2 = players.length; i2 < len2; i2++){
        ...
        if(timeUp){
          ...
          if(healthComp == undefined || hungerComp == undefined) continue
          memoryRaidFailed(player, healthComp, hungerComp)
        } else {
          // player.onScreenDisplay.setActionBar(roomProgress + "/" + roomGoal + " = " + Math.floor(roomProgress / roomGoal * 200) + " / " + completed + " / " + room + " / " + isMiniBoss + roomOffset + " / " + roomId)
  ...
  if(dimensionsLength == 0) return

  system.run(() => { runInterval(execution >= 4 ? 0 : execution +1) })
}

export function startInterval(dimension: Dimension, reload = false): void {
  const score = apiScoreboard.getObj(dimension.id)

  if(reload){
    if(!score.hasParticipant("timer")) return
...
export function clearTimerInterval(dimension: Dimension): void {
  const cache = timerIntervalId[dimension.id]
  if(cache) cache.stop = true
}
