import { Dimension, ScoreboardObjective, StructureRotation } from "@minecraft/server"
...
import { IDoorInfo } from "./memory/rooms"

export function startUpCacheFromScore(dimension: Dimension, score: ScoreboardObjective){
  const roomsRaw: string[] = []
  const exits: IDoorInfo[] = []
  ...
  for(let i = 0, len = participants.length; i < len; i++){
    ...
    if(prefix == "e-"){
      ...
      if(rot == undefined || rawX == undefined || rawY == undefined || rawZ == undefined) continue

      const x = parseInt(rawX), y = parseInt(rawY), z = parseInt(rawZ)
      exits.push({dir: rot as StructureRotation, x, y, z})
      continue
    }
  }
  ...
  for(let i = 0, len = sortedRooms.length; i < len; i++){
    ...
    if(rawX1 == undefined || rawX2 == undefined || rawY1 == undefined || rawY2 == undefined || rawZ1 == undefined || rawZ2 == undefined) continue
  ...
  // console.warn("Aqui coloca o controller das função pra ficar melhor já que só tem um evento por dimensão")
  startEventType0KillAll(dimension, true)

  // Retoma o timer da raid
